...
public interface EnrollRepository extends JpaRepository<EnrollEntity, Long> {
    ...
    @Query("SELECT NEW unicauca.edu.co.sga.common_utilities_service.application.dto.response.StudentView.SubjectResponseViewDTO(" +
            "s.name, t.name) " +
            "FROM EnrollEntity e " +
            "JOIN e.course c " +
            "JOIN c.subject s " +
    ...
    @Query("SELECT new unicauca.edu.co.sga.common_utilities_service.application.dto.response.StudentView.RubricResponseViewDTO(r.nombreRubrica) " +
            "FROM EnrollEntity e " +
            "JOIN e.course c " +
            "JOIN c.subject s " +
            "JOIN s.rubric r " +
...
//    @Query("SELECT new unicauca.edu.co.sga.common_utilities_service.application.dto.response.StudentView.RubricResponseViewDTO(r.name, r.created_at) " +
//            "FROM EnrollEntity e " +
//            "JOIN e.course c " +
//            "JOIN c.subject s " +
//            "JOIN s.rubric r " +
