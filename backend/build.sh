...
tmux send-keys -t auth-service:0 './environment.sh' C-m 'cd evaluation-service' C-m 'mvn spring-boot:run' C-m

# Iniciar audit-service con perfil prod
tmux new-session -d -s ModuloRubricaCriterio_Hexagonal-service
tmux send-keys -t audit-service:0 './environment.sh' C-m 'cd ModuloRubricaCriterio_Hexagonal' C-m 'mvn spring-boot:run' C-m
